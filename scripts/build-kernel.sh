...
if [[ "${MAINLINE}" != "Y" ]]; then
    test -d linux-rockchip || git clone --single-branch --progress -b linux-5.10-gen-rkr6 https://github.com/Joshua-Riek/linux-rockchip.git --depth=1 linux-rockchip
    cd linux-rockchip
    ...
    test -d linux ||  git clone --single-branch --progress -b v6.5-rk3588 https://github.com/SandroDickens/linux-Joshua-Riek.git --depth=1 linux
    cd linux
