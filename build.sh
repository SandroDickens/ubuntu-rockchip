...
for i in "$@"; do
    case $i in
        ...
        -b|--board)
            ...
            ;;
        -d|--docker)
            DOCKER="podman run --privileged --network=host --rm -it -v \"$(pwd)\":/opt -e BOARD -e VENDOR -e LAUNCHPAD ubuntu-rockchip-build /bin/bash"
            podman build -t ubuntu-rockchip-build docker
            shift
            ;;
        -k|--kernel-only)
